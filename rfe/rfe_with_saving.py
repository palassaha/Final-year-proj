...
def main():
    X, y, feature_names, original_df = load_data('dataset/augmented_dataset.csv')
    ...
    # Save best model's selected features with status
    best_model = max(results.items(), key=lambda x: x[1]['accuracy'])[0]
    best_features = results[best_model]['selected_features']
    print(f"\nBest model based on accuracy: {best_model}")
    print(f"Saving top 5 features: {best_features} along with 'status'...")

    columns_to_save = best_features + ['status']
    selected_df = original_df[columns_to_save]
    selected_df.to_csv(r'dataset\top_features_selected.csv', index=False)
    print("Saved selected features to top_features_selected.csv")
